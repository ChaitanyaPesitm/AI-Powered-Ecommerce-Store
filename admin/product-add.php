...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$errors) {
    $st = $pdo->prepare("INSERT INTO products (category_id, name, description, specifications, price, stock, image, model_glb) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $st->execute([$cat, $name, $desc, $specs, $price, $stock, $img, $model_glb]);
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
      ...
      <div>
        <div class="form-group">
          ...
          <input type="text" name="name" class="form-input" placeholder="Enter product name" required value="<?= htmlspecialchars($_POST['name'] ?? '') ?>">
        </div>
        ...
        <div class="form-group">
          ...
          <input type="number" name="price" class="form-input" placeholder="0.00" step="0.01" min="0" required value="<?= htmlspecialchars($_POST['price'] ?? '') ?>">
        </div>
        ...
        <div class="form-group">
          ...
          <input type="number" name="stock" class="form-input" placeholder="0" min="0" value="<?= htmlspecialchars($_POST['stock'] ?? '0') ?>">
        </div>
      </div>
      ...
      <div>
        <div class="form-group">
          ...
          <textarea name="description" class="form-textarea" placeholder="Enter product description" rows="4"><?= htmlspecialchars($_POST['description'] ?? '') ?></textarea>
        </div>
        ...
        <div class="form-group">
          ...
          <textarea name="specifications" class="form-textarea" placeholder="Enter product specifications (e.g., Color: Red, Size: M)" rows="4"><?= htmlspecialchars($_POST['specifications'] ?? '') ?></textarea>
        </div>
        ...
        <div class="form-group">
          ...
          <div style="position: relative;">
            ...
            <div id="imagePreview" style="margin-top: 15px; display: none;">
              <img id="previewImg" src="" style="max-width: 200px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
            </div>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <small class="text-muted">Upload a .glb file for 3D view</small>
        </div>
      </div>
    </div>
